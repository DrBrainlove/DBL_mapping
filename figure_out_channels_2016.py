...
NUM_MODULES=8
MAX_CHANNELS_TOTAL=144
MAX_LEDS_PER_CHANNEL=220
...
for module in range(1,9):
    ...
    for x in range(0,100000):
        if x % 10000==0:
            print module,x,datetime.datetime.now()
        ...
        if len(bars_remaining)<4 and jumps<3:
            print module,"YAY"
            ...
            json.dump(dumptojson,open("possible_mappings_2016/"+"M"+str(module)+"_R"+str(distinct_receivernodes)+"_B"+str(len(bars_remaining))+"_J"+str(jumps)+"_G"+str(groundnode_utilization)+"_"+nowstamp+"_A"+str(num_alternative_startnodes)+"_"+mapping_name+".json","w"))
